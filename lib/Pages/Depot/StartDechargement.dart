...
class _StartDechargementState extends State<StartDechargement> {
  ...
  vehiculePosition() async {
    var data = await RemoteStationService.staionGetVehiculPosition(
      camion: widget.camion_id,
    );
    setState(() {
    ...
    if (result['data']['station']['id'] == prefUserInfo['stationId']) {
      if (potUserLat != potStationLat ||
        ...
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder:
                ((context) => ConfirmBL(data: _detailBL, bl: widget.bl_id)),
          ),
        );
      }
      ...
      erroDestination(
        "La destination n'est pas connue. Conatctez la société pétrolière",
      );
  ...
  recapFunctionFalsecommunication() async {
    ...
    if (result['data']['station']['id'] == prefUserInfo['stationId']) {
      if (potUserLat != potStationLat ||
        ...
        result['data']['station']['id'] == null ||
        result['data']['station']['id'] == "" ||
        result['data']['station']['nom'] == 'Destination inconnue') {
      erroDestination(
        "La destination n'est pas connue. Conatctez la société pétrolière",
      );
    } else if (result['data']['station']['id'] != prefUserInfo['stationId']) {
