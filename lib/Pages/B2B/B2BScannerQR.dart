...
class B2BScannerQR extends StatefulWidget {
  const B2BScannerQR({Key? key}) : super(key: key);

  @override
  State<B2BScannerQR> createState() => _B2BScannerQRState();
}

class _B2BScannerQRState extends State<B2BScannerQR> {
  int _counter = 5;
  int id = 0;
  int id_bl = 0;
  late Timer _timer;
  ...
  recapFunction() async {
    ...
    if (result['data']['station']['id'] == prefUserInfo['b2bId']) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: ((context) => B2bConfirmBL(detail: detailBL, bl: id_bl)),
        ),
      );
    } else {
      erroDestination(
